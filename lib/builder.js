...
exports.build = async function (CONFIG, ACTIVE_BRANCH) {
    ...
    try {
        ACTIVE_BRANCH = ACTIVE_BRANCH || (require('git-branch').sync() || '').replace(/\//g, '_');
    } catch (err) {
        // NOTE: We purposely silence this error.
    }
    ACTIVE_BRANCH = ACTIVE_BRANCH || GITHUB_REF || 'master';
...
//            "firefoxVersion": "firefoxdeveloperedition",
    ...
    FS.unlinkSync(PATH.join(buildBasePath, "scripts/page-action.html"));
    FS.unlinkSync(PATH.join(buildBasePath, "scripts/page-action.js"));
    ...
    return GET_PORT().then(async function (PORT) {
        ...
        if (
            normalizedManifest.page &&
            normalizedManifest.page.action
        ) {
            scripts.page_action = LODASH.cloneDeep(normalizedManifest.page.action);
            delete normalizedManifest.page;
        }
        ...
        */
        ...
        if (normalizedManifest.files) {
            Object.keys(normalizedManifest.files).forEach(function (targetPath) {
                FS.copySync(normalizedManifest.files[targetPath], PATH.join(buildBasePath, targetPath));
            });
            delete normalizedManifest.files;
        }
        ...
        function mapScript (scriptType, script) {
            ...
            if (/\.js$/.test(scriptFilepath)) {
                ...
                if (scriptType === "page_action") {
                    ...
                    scriptFilenames[scriptType].push({
                        // We use the extension name if not specified
                        label: scriptDeclarations.label || normalizedManifest.short_name || normalizedManifest.name || "My Action",
                        // We use the extension icon for now
                        icons: normalizedManifest.icons,
            ...
            if (typeof scriptDeclarations === 'string') {

                if (scriptType === "page_action") {
                    scriptFilenames[scriptType][0].filename = scriptDeclarations;
                }
    ...
    */
        ...
        if (scripts.page_action) {

            var info = mapScript("page_action", scripts.page_action);

            FS.copySync(
                PATH.join(templateBasePath, "scripts/page-action.html"),
                PATH.join(buildBasePath, "scripts", info.scriptFilepath.replace(/\.js/, ".html"))
            );
            ...
            FS.copySync(
                PATH.join(templateBasePath, "scripts/page-action.js"),
                PATH.join(buildBasePath, "scripts", info.scriptFilepath.replace(/\.js/, ".loader.js"))
            );
            ...
            // mapScript("page_action", scripts.page_action);

            normalizedManifest.page_action = {
                "default_title": scriptFilenames["page_action"].label,
                "default_popup": PATH.join("scripts", info.scriptFilepath.replace(/\.js/, ".html"))
            };
            if (scriptFilenames["page_action"][0].icons) {
                normalizedManifest.page_action.default_icon = scriptFilenames["page_action"][0].icons;
            }
        }

//console.log("scriptFilenames", scriptFilenames);

        // replaceInFile(PATH.join(buildBasePath, "page-action.js"), {
        //     "SCRIPT": JSON.stringify(scriptFilenames["page_action"][0].filename)
        // });
        // if (!normalizedManifest.page_action) {
        //     FS.removeSync(PATH.join(buildBasePath, "page-action.html"));
        //     FS.removeSync(PATH.join(buildBasePath, "page-action.js"));
        // }

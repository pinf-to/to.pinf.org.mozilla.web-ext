...
GET_PORT().then(async function (PORT) {
    ...
    function mapScript (scriptType, script) {
        ...
        if (
            ...
            Object.keys(scriptDeclarations.include).forEach(function (name) {
                ...
                if (process.env.VERBOSE) console.log("copy include", RESOLVE.sync(scriptDeclarations.include[name], { basedir: process.cwd() }), PATH.join(buildBasePath, 'dist', name));
